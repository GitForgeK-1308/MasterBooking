...
async def test_get_categories_returns_only_active_sorted(
    ...
    assert str(inactive_category.id) not in {item["id"] for item in data}
...
async def test_get_category_tree(
    ...
    response = await ac.get("/categories/tree")
    ...
    assert root["id"] == str(category.id)
    ...
    assert child["id"] == str(child_category.id)
    assert child["name"] == "Hair"
    assert child["parent_id"] == str(category.id)

    assert len(child["children"]) == 1

    grandchild = child["children"][0]

    assert grandchild["id"] == str(grandchild_category.id)
    assert grandchild["name"] == "Coloring"
    assert grandchild["parent_id"] == str(child_category.id)
