...
async def test_get_images_by_offering_id_sorted(
    ...
    result = (
        await repository.get_by_offering_id(
            offering.id
        )
    )
...
async def test_count_images_by_offering_id(
    ...
    result = (
        await repository.count_by_offering_id(
            offering.id
        )
    )
...
async def test_get_primary_image(
    ...
    result = await repository.get_primary(
        offering.id
    )
...
async def test_update_image(
    ...
    assert (
        image_from_database.sort_order
        == 7
    )
...
async def test_set_primary_image(
    ...
    old_primary = await repository.get_by_id(
        offering_image.id
    )
...
async def test_delete_primary_image_promotes_next(
    ...
    await repository.delete_with_primary_fallback(
        offering_image
    )

    deleted_image = await repository.get_by_id(
        primary_id
    )

    assert deleted_image is None

    new_primary = await repository.get_primary(
        offering.id
    )
...
async def test_delete_last_primary_leaves_no_primary(
    ...
    count = await repository.count_by_offering_id(
        offering.id
    )
