...
async def test_get_my_reviews(
    ...
    assert all(item["offering_id"] == str(offering.id) for item in data)

    assert all(item["offering_title"] == "Classic Cut" for item in data)

    assert data[0]["client_name"] == "Удалённый пользователь"

    assert data[1]["client_name"] == "Ivan Ivanov"

    assert data[2]["client_name"] == "Ivan Ivanov"

    assert str(foreign_master_review.id) not in {item["id"] for item in data}
...
async def test_get_my_reviews_scoped_to_current_master(
    ...
    assert data[0]["id"] == str(foreign_master_review.id)

    assert data[0]["offering_id"] == str(second_master_offering.id)

    assert data[0]["offering_title"] == "Nail Care"

    assert str(review.id) not in {item["id"] for item in data}
...
async def test_get_my_reviews_without_token(
    ...
    response = await ac.get("/masters/me/reviews")
